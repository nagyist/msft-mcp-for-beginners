...
Puiku, tai reiškia, kad viskas paleista teisingai.

## Išbandykite serverį

Patikrinkite funkcionalumą naudodami šią komandą:

```sh
npx @modelcontextprotocol/inspector node build/app.js
```

Tai turėtų paleisti inspektoriaus įrankio internetinę sąsają.
...
Turėtumėte gauti atsakymą, panašų į:
...
Pabandykite išbandyti neegzistuojantį įrankį, pvz., „add2“, naudodami šią komandą:

```sh
npx @modelcontextprotocol/inspector --cli node ./build/app.js --method tools/call --tool-name add2 --tool-arg a=1 --tool-arg b=2
```

Dabar turėtumėte matyti šį pranešimą, rodantį, kad jūsų validacija veikia:
...
Taip pat pabandykite nusiųsti parametrą `c`, kuris turėtų būti atmestas pagal schemą, pvz.:

```sh
npx @modelcontextprotocol/inspector --cli node ./build/app.js --method tools/call --tool-name add --tool-arg a=1 --tool-arg c=2
```

Dabar turėtumėte matyti klaidą „neteisingi argumentai“:
