...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-11T14:43:46+00:00",
...
ఈ ప్రయోగశాలలో మీరు AI సహాయకులకు శక్తివంతమైన డేటాబేస్ క్వెరీ సామర్థ్యాలు, స్కీమా ఇంట్రోస్పెక్షన్, మరియు విశ్లేషణా ఫంక్షన్లను అందించే సొగసైన MCP టూల్స్ సృష్టించడంలో లోతుగా నేర్చుకుంటారు. మీరు శక్తివంతమైన మరియు సురక్షితమైన టూల్స్‌ను నిర్మించడం నేర్చుకుంటారు, సమగ్ర లోప నిర్వహణ మరియు పనితీరు ఆప్టిమైజేషన్‌తో.

## అవలోకనం

MCP టూల్స్ AI సహాయకులు మరియు మీ డేటా వ్యవస్థల మధ్య ఇంటర్‌ఫేస్. బాగా రూపకల్పన చేసిన టూల్స్ సంక్లిష్ట ఆపరేషన్లకు నిర్మిత, ధృవీకరించబడిన యాక్సెస్‌ను అందిస్తాయి, అదే సమయంలో భద్రత మరియు పనితీరు నిలుపుకుంటాయి. ఈ ప్రయోగశాల టూల్ అభివృద్ధి యొక్క పూర్తి జీవనచక్రాన్ని డిజైన్ నుండి డిప్లాయ్‌మెంట్ వరకు కవర్ చేస్తుంది.

మా రిటైల్ MCP సర్వర్ విక్రయ డేటా, ఉత్పత్తి క్యాటలాగ్లు, మరియు వ్యాపార విశ్లేషణల సహజ భాషా క్వెరీలను అనుమతించే సమగ్ర టూల్ సూట్‌ను అమలు చేస్తుంది, కఠిన భద్రతా సరిహద్దులు మరియు ఉత్తమ పనితీరు నిలుపుకుంటూ.

## నేర్చుకునే లక్ష్యాలు

ఈ ప్రయోగశాల ముగిసిన తర్వాత, మీరు చేయగలుగుతారు:

- **డిజైన్ చేయండి** సంక్లిష్ట పారామీటర్ ధృవీకరణతో అధునాతన MCP టూల్స్  
- **అమలు చేయండి** SQL ఇంజెక్షన్ రక్షణతో సురక్షిత డేటాబేస్ క్వెరీ టూల్స్  
- **సృష్టించండి** డైనమిక్ క్వెరీల కోసం స్కీమా ఇంట్రోస్పెక్షన్ సామర్థ్యాలు  
- **నిర్మించండి** వ్యాపార బుద్ధిమత్త కోసం కస్టమ్ విశ్లేషణా టూల్స్  
- **అన్వయించండి** సమగ్ర లోప నిర్వహణ మరియు సున్నితమైన తగ్గింపు  
- **ఆప్టిమైజ్ చేయండి** ఉత్పత్తి వర్క్‌లోడ్‌ల కోసం టూల్ పనితీరు  

## 🛠️ ప్రాథమిక టూల్ నిర్మాణం

### టూల్ డిజైన్ సూత్రాలు
...
class BaseTool(ABC):
    ...
    async def call(self, **kwargs) -> ToolResult:
        ...
        try:
            # ఇన్‌పుట్ పరామితులను ధృవీకరించండి
            self._validate_input(kwargs)
            
            # టూల్ అమలును లాగ్ చేయండి
            logger.info(
            ...
            # అమల సమయాన్ని నమోదు చేయండి
            execution_time = (time.time() - start_time) * 1000
            result.execution_time_ms = execution_time
            self.total_execution_time += execution_time
            
            # విజయాన్ని లాగ్ చేయండి
            logger.info(
    ...
    def _validate_input(self, kwargs: Dict[str, Any]):
        ...
        # అవసరమైన పరామితులను తనిఖీ చేయండి
        ...
        # టైప్ ధృవీకరణ ఇక్కడ జరుగుతుంది
        # ఉత్పత్తి కోసం, సమగ్ర ధృవీకరణకు jsonschema లైబ్రరీని ఉపయోగించండి
    
    def _sanitize_parameters(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        """Sanitize parameters for logging (remove sensitive data)."""
        
        # సున్నితమైన పరామితులను తొలగించండి లేదా మాస్క్ చేయండి
...
class DatabaseTool(BaseTool):
    ...
    async def execute_query(
        ...
        async with self.get_connection() as conn:
            try:
                # అందించినట్లయితే స్టోర్ సందర్భాన్ని సెట్ చేయండి
                if store_id:
...
class QueryValidator:
    ...
    # ప్రమాదకరమైన SQL కీవర్డ్లు మరియు నమూనాలు
    DANGEROUS_KEYWORDS = {
    ...
    # అనుమతించబడిన చదవ-only ఆపరేషన్లు
    SAFE_KEYWORDS = {
    ...
    # అనుమతించబడిన స్కీమాలు మరియు పట్టికలు
    ...
    def _validate_table_access(self, query: str, validation_result):
        ...
        # పట్టిక పేర్లను తీసుకోండి (సరళీకృత విధానం)
        # ఉత్పత్తిలో, సరైన SQL పార్సింగ్ ఉపయోగించండి
        ...
        for table_ref in all_tables:
            if '.' in table_ref:
                ...
                # పేర్కొనబడకపోతే రిటైల్ స్కీమాను అనుమానించండి
...
# గ్లోబల్ వాలిడేటర్ ఉదాహరణ
query_validator = QueryValidator()
```

## 🗃️ డేటాబేస్ క్వెరీ టూల్స్

### విక్రయ విశ్లేషణ టూల్
...
class SalesAnalysisTool(DatabaseTool):
    ...
    def __init__(self, db_provider):
        ...
        # సాధారణ విశ్లేషణ కోసం ముందుగా తయారుచేసిన క్వెరీ టెంప్లేట్లు
    ...
    async def _execute_custom_query(self, kwargs: Dict[str, Any]) -> ToolResult:
        ...
        # భద్రత కోసం క్వెరీని ధృవీకరించండి
        ...
        # ధృవీకరించిన క్వెరీని అమలు చేయండి
...
class SchemaIntrospectionTool(DatabaseTool):
    ...
    async def _get_single_table_schema(
        ...
        async with self.get_connection() as conn:
            ...
            # అభ్యర్థించబడితే పరిమితులు పొందండి
            ...
            # అభ్యర్థించబడితే సూచికలు పొందండి
            if include_indexes:
                indexes_query = """
                    SELECT 
            ...
            # అభ్యర్థించబడితే పట్టిక గణాంకాలు పొందండి
    ...
    async def _get_all_tables_schema(
        ...
        async with self.get_connection() as conn:
            # రిటైల్ స్కీమాలోని అన్ని పట్టికలను పొందండి
            tables_query = """
                SELECT 
                    table_name,
                    table_type
            ...
            for table in tables:
                ...
                # ప్రతి పట్టికకు కాలమ్స్ పొందండి
                columns_query = """
                    SELECT 
                        column_name,
                        data_type,
...
## 📊 విశ్లేషణ మరియు ఉపకరణ టూల్స్

### వ్యాపార బుద్ధిమత్త టూల్
...
class BusinessIntelligenceTool(DatabaseTool):
    ...
    async def _generate_business_summary(self, kwargs: Dict[str, Any]) -> ToolResult:
        ...
        if result.success and result.data:
            ...
            # ఉత్పన్నమైన అవగాహనలను జోడించండి
            insights = {
                'revenue_trend': 'stable',  # చారిత్రక డేటా ఆధారంగా లెక్కించబడుతుంది
...
✅ **అధునాతన టూల్ నిర్మాణం**: సమగ్ర లోప నిర్వహణతో సొగసైన MCP టూల్స్ అమలు చేశారు  
✅ **క్వెరీ ధృవీకరణ**: ఇంజెక్షన్ దాడులను నివారించడానికి సురక్షిత SQL ధృవీకరణ నిర్మించారు  
✅ **డేటాబేస్ టూల్స్**: శక్తివంతమైన విక్రయ విశ్లేషణ మరియు స్కీమా ఇంట్రోస్పెక్షన్ సామర్థ్యాలు సృష్టించారు  
✅ **వ్యాపార బుద్ధిమత్త**: సమగ్ర వ్యాపార అవగాహన కోసం విశ్లేషణా టూల్స్ అభివృద్ధి చేశారు  
✅ **పనితీరు ఆప్టిమైజేషన్**: క్యాషింగ్, కనెక్షన్ పూలింగ్, మరియు క్వెరీ ఆప్టిమైజేషన్ అన్వయించారు  
✅ **భద్రతా సమీకరణ**: పాత్ర ఆధారిత యాక్సెస్ నియంత్రణ మరియు ఆడిట్ లాగింగ్ అమలు చేశారు  

## 🚀 తదుపరి ఏమిటి

**[Lab 07: Semantic Search Integration](../07-Semantic-Search/README.md)** తో కొనసాగండి:

- MCP టూల్స్‌తో వెక్టర్ సెర్చ్ సామర్థ్యాలను సమీకరించండి  
- సేమాంటిక్ ఉత్పత్తి శోధన ఫంక్షనాలిటీని నిర్మించండి  
- AI ఆధారిత క్వెరీ అర్థం చేసుకోవడాన్ని అమలు చేయండి  
- సాంప్రదాయ మరియు వెక్టర్ క్వెరీలను కలిపిన హైబ్రిడ్ శోధన సృష్టించండి  

## 📚 అదనపు వనరులు

### MCP టూల్ అభివృద్ధి
- [Model Context Protocol Documentation](https://modelcontextprotocol.io/docs) - అధికారిక MCP స్పెసిఫికేషన్  
- [FastMCP Framework](https://github.com/jlowin/fastmcp) - Python MCP అమలు  
- [MCP Tool Patterns](https://github.com/modelcontextprotocol/servers) - ఉదాహరణ టూల్ అమలు  

### డేటాబేస్ భద్రత
- [SQL Injection Prevention](https://owasp.org/www-community/attacks/SQL_Injection) - OWASP భద్రతా గైడ్  
- [PostgreSQL Security](https://www.postgresql.org/docs/current/security.html) - డేటాబేస్ భద్రతా ఉత్తమ పద్ధతులు  
- [Query Validation Techniques](https://cheatsheetseries.owasp.org/cheatsheets/Query_Parameterization_Cheat_Sheet.html) - సురక్షిత క్వెరీ నమూనాలు  

### పనితీరు ఆప్టిమైజేషన్
- [Database Query Optimization](https://www.postgresql.org/docs/current/performance-tips.html) - PostgreSQL పనితీరు గైడ్  
- [Connection Pooling Best Practices](https://www.postgresql.org/docs/current/runtime-config-connection.html) - కనెక్షన్ నిర్వహణ  
- [Async Python Patterns](https://docs.python.org/3/library/asyncio.html) - అసింక్రోనస్ ప్రోగ్రామింగ్ గైడ్  

---

**మునుపటి**: [Lab 05: MCP Server Implementation](../05-MCP-Server/README.md)  
**తదుపరి**: [Lab 07: Semantic Search Integration](../07-Semantic-Search/README.md)
