...
Model Context Protocol (MCP) 提供傳輸機制的彈性，允許在專門的企業環境中實作自訂方案。本進階指南以 Azure Event Grid 和 Azure Event Hubs 為實例，探討如何建構可擴充、雲端原生的 MCP 解決方案。

## 介紹

雖然 MCP 的標準傳輸（stdio 和 HTTP 串流）可滿足大多數使用情境，企業環境常需專門的傳輸機制以提升擴充性、可靠性，並整合現有雲端基礎架構。自訂傳輸使 MCP 能利用雲端原生的訊息服務，實現非同步通訊、事件驅動架構及分散式處理。
...
- **建置 Azure Event Grid 傳輸**：利用 Azure Event Grid 建立事件驅動的 MCP 伺服器，實現無伺服器擴充
- **實作 Azure Event Hubs 傳輸**：設計高吞吐量 MCP 解決方案，使用 Azure Event Hubs 實現即時串流
- **應用企業模式**：將自訂傳輸整合至現有 Azure 基礎架構與安全模型
- **處理傳輸可靠性**：實作訊息持久性、排序及錯誤處理以符合企業需求
- **優化效能**：設計符合規模、延遲與吞吐量需求的傳輸方案
...
Azure Event Grid 提供無伺服器事件路由服務，適合事件驅動的 MCP 架構。本實作示範如何建構可擴充、鬆耦合的 MCP 系統。
...
1. **冪等性**：確保訊息處理具冪等性以應對重複
2. **錯誤處理**：實作完整錯誤處理與死信佇列
3. **監控**：加入詳細遙測與健康檢查
4. **安全性**：使用管理身分與最小權限存取
5. **效能**：依特定延遲與吞吐需求設計
...
自訂 MCP 傳輸可利用 Azure 訊息服務實現強大企業場景。透過實作 Event Grid 或 Event Hubs 傳輸，您能建構可擴充、可靠且與現有 Azure 基礎架構無縫整合的 MCP 解決方案。

範例展示了生產環境可用的自訂傳輸模式，同時保持 MCP 協定合規與 Azure 最佳實務。
...
> *本指南聚焦於生產環境 MCP 系統的實作模式。請務必根據您的具體需求與 Azure 服務限制驗證傳輸實作。*
> **現行標準**：本指南反映了 [MCP 規範 2025-06-18](https://spec.modelcontextprotocol.io/specification/2025-06-18/) 的傳輸需求及企業環境的進階傳輸模式。
