...
Dis hands-on lab go show you how to build MCP server wey fit work for production using FastMCP framework. You go build di main server structure, join am with database, create tools to access data, and set di foundation for AI-powered retail analytics.

## Overview

Di MCP server na di main thing for our retail analytics solution. E dey act like bridge between AI assistants and di PostgreSQL database, wey dey provide secure and smart access to business data through one standard protocol.

Dis lab go teach you how to build strong and scalable MCP server wey follow enterprise patterns and best practices.

## Learning Objectives

By di time you finish dis lab, you go sabi:

- **Build** FastMCP server wey get correct architecture and organization  
- **Implement** database integration wey get connection pooling and error handling  
- **Create** MCP tools to check database schema and run query  
...
Make we check how dem arrange di MCP server:
...
First, make we create one strong configuration system:
...
- **Type Safety**: Validation and type hints with dataclass  
- **Flexible Defaults**: Good defaults for development  
- **Validation**: Validation wey dey give helpful error messages  
- **Security**: Sensitive values go only come from environment variables  

## 🗄️ Database Integration Layer

### PostgreSQL Provider (`sales_analysis_postgres.py`)

Make we implement di database integration layer:
...
- **Connection Pooling**: Manage resources well with asyncpg  
- **RLS Integration**: Automatic Row Level Security context setting  
- **Schema Introspection**: Discover table schema dynamically  
- **Error Handling**: Manage errors well and log dem  
- **Query Formatting**: Format results wey AI go fit understand  
- **Health Monitoring**: Check database connectivity and pool status  

## 🔧 Main MCP Server Implementation

### FastMCP Server (`sales_analysis.py`)

Now, make we implement di main MCP server:
...
- **Tool Registration**: Define tools with type safety  
- **RLS Context Management**: Extract user identity and set context automatically  
- **Error Handling**: Manage errors well with user-friendly messages  
- **Lifecycle Management**: Start and stop di server well with resource cleanup  
- **Health Monitoring**: Health check endpoints dey built-in  
- **Development Support**: Hot reload and debugging dey available  
...
2. **Test am for AI Chat**:  
   - Open VS Code AI Chat  
   - Type `#zava` and select your server  
   - Ask: "Wetin be di tables wey dey available?"  
   - Ask: "Show me di top 5 stores by number of orders"  

### Unit Testing

Create unit tests wey cover everything:
...
✅ **Working MCP Server**: FastMCP server wey don join with database  
✅ **Configuration Management**: Strong environment-based configuration  
✅ **Database Layer**: PostgreSQL integration wey get connection pooling  
✅ **MCP Tools**: Tools to check schema and run query  
✅ **RLS Integration**: Row Level Security context management  
✅ **Health Monitoring**: Health check endpoints wey complete  
...
- Create special analytics tools  
...
- [Tool Development Guide](https://modelcontextprotocol.io/docs/tools/) - How to create MCP tools  

### Database Integration
- [asyncpg Documentation](https://magicstack.github.io/asyncpg/current/) - PostgreSQL async driver  
- [Connection Pooling Best Practices](https://www.postgresql.org/docs/current/runtime-config-connection.html) - How to tune PostgreSQL  
- [Row Level Security Guide](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - How to implement RLS  
...
- [Background Tasks](https://fastapi.tiangolo.com/tutorial/background-tasks/) - How to manage async tasks  

---

**Next**: Ready to add more tools? Continue with [Lab 06: Tool Development](../06-Tools/README.md)
