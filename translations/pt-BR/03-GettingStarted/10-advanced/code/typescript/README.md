...
Ótimo, isso significa que está iniciando corretamente.
...
Isso deve iniciar a interface web da ferramenta de inspeção.
...
Tente testar uma ferramenta que não existe, como "add2", com este comando:

```sh
npx @modelcontextprotocol/inspector --cli node ./build/app.js --method tools/call --tool-name add2 --tool-arg a=1 --tool-arg b=2
```

Agora você deve ver esta mensagem mostrando que sua validação funciona:
...
Também tente enviar um parâmetro `c` que deve ser rejeitado pelo esquema, como:

```sh
npx @modelcontextprotocol/inspector --cli node ./build/app.js --method tools/call --tool-name add --tool-arg a=1 --tool-arg c=2
```

Agora você deve ver o erro "argumentos inválidos":
