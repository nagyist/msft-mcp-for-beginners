...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-11T05:22:10+00:00",
...
把 MCP 想像成 AI 應用程式的萬用翻譯器——就像 USB 埠讓您能將任何裝置連接到電腦一樣，MCP 讓 AI 模型能以標準化方式連接任何工具或服務。無論您是在打造第一個聊天機器人，還是在處理複雜的 AI 工作流程，理解 MCP 都會賦予您創造更強大且靈活應用程式的能力。

本課程設計耐心且細心，陪伴您的學習旅程。我們將從您已熟悉的簡單概念開始，並透過您喜愛的程式語言逐步建立專業知識。每個步驟都包含清楚的說明、實用範例，以及豐富的鼓勵。
...
- 🧑‍💻 [MCP GitHub 儲存庫](https://github.com/modelcontextprotocol) – 這裡有多種程式語言的 SDK、工具和程式碼範例。就像實用範例和現成元件的寶庫。
...
• **理解 MCP 基礎**：您將掌握模型上下文協定是什麼，以及為何它正在革新 AI 應用程式的協作方式，並透過類比和範例讓概念更易理解。
...
• **自信部署**：您將知道如何將 MCP 專案從開發帶到生產環境，並掌握實務可行的部署策略。
...
MCP 是一種協定——一套約定規則，幫助 AI 模型和應用程式與工具和服務進行有效「對話」。就像對話規則讓人類溝通更順暢，MCP 讓 AI 應用溝通更可靠且強大。
...
想像如果每家車廠的加油槍形狀都不同——您就得為每台車準備不同轉接頭！標準化就是同意共通方式，讓事物能無縫協作。

MCP 為 AI 應用提供這種標準化。不是每個 AI 模型都要寫客製化程式碼來配合每個工具，MCP 創造一種通用溝通方式。這讓開發者能一次打造工具，並讓它們與多種 AI 系統相容。

## 🧭 您的學習路徑概覽

您的 MCP 旅程經過精心規劃，逐步建立信心與技能。每個階段介紹新概念，同時強化已學內容。

### 🌱 基礎階段：理解基本概念（模組 0-2）

冒險從這裡開始！我們將用熟悉的類比和簡單範例介紹 MCP 概念。您會了解 MCP 是什麼、為何存在，以及它如何融入 AI 開發的宏觀世界。

• **模組 0 - MCP 介紹**：我們將探索 MCP 是什麼，以及它為何對現代 AI 應用如此重要。您會看到 MCP 實際運作的範例，並理解它如何解決開發者常見問題。

• **模組 1 - 核心概念說明**：這裡您將學習 MCP 的基本構成要素。我們會用大量類比和視覺範例，確保這些概念自然且易懂。

• **模組 2 - MCP 的安全性**：安全聽起來可能令人畏懼，但我們會展示 MCP 內建的安全功能，並教您從一開始就保護應用程式的最佳實務。

### 🔨 建構階段：打造您的第一個實作（模組 3）

真正的樂趣現在開始！您將親手打造 MCP 伺服器和用戶端。別擔心——我們會從簡單開始，逐步引導您完成每一步。
...
您還會學習如何將 MCP 實作擴展到生產環境，並與 Azure 等雲端平台整合。這些模組讓您能打造能應付真實需求的 MCP 解決方案。

### 🌟 精通階段：社群與專精（模組 6-11）
最後階段著重於加入 MCP 社群並專精於你最感興趣的領域。你將學習如何為開源 MCP 專案做出貢獻、實作進階的身份驗證模式，以及建構完整的資料庫整合解決方案。

第 11 模組值得特別一提——這是一個完整的 13 個實驗室的實作學習路徑，教你如何建置具備 PostgreSQL 整合的生產級 MCP 伺服器。它就像是一個總結專案，將你所學的一切串連起來！
...
| 5.7 | 擴展 | MCP 實作的擴展 | [指南](./05-AdvancedTopics/mcp-scaling/README.md) |
...
| 07 | 早期採用見解 | 實際應用故事 | [指南](./07-LessonsFromEarlyAdoption/README.md) |
...
| **模組 11：MCP 伺服器實作實驗室** | | | |
| 11 | MCP 伺服器資料庫整合 | 13 個實驗室的完整 PostgreSQL 整合實作路徑 | [實驗室](./11-MCPServerHandsOnLabs/README.md) |
| 11.1 | 簡介 | MCP 與資料庫整合及零售分析案例概述 | [實驗室 00](./11-MCPServerHandsOnLabs/00-Introduction/README.md) |
| 11.2 | 核心架構 | 理解 MCP 伺服器架構、資料庫層與安全模式 | [實驗室 01](./11-MCPServerHandsOnLabs/01-Architecture/README.md) |
| 11.3 | 安全性與多租戶 | 行級安全、身份驗證與多租戶資料存取 | [實驗室 02](./11-MCPServerHandsOnLabs/02-Security/README.md) |
| 11.4 | 環境設定 | 開發環境、Docker、Azure 資源設定 | [實驗室 03](./11-MCPServerHandsOnLabs/03-Setup/README.md) |
| 11.5 | 資料庫設計 | PostgreSQL 設定、零售架構設計與範例資料 | [實驗室 04](./11-MCPServerHandsOnLabs/04-Database/README.md) |
| 11.6 | MCP 伺服器實作 | 建置具資料庫整合的 FastMCP 伺服器 | [實驗室 05](./11-MCPServerHandsOnLabs/05-MCP-Server/README.md) |
| 11.7 | 工具開發 | 建立資料庫查詢工具與架構檢視 | [實驗室 06](./11-MCPServerHandsOnLabs/06-Tools/README.md) |
| 11.8 | 語意搜尋 | 使用 Azure OpenAI 與 pgvector 實作向量嵌入 | [實驗室 07](./11-MCPServerHandsOnLabs/07-Semantic-Search/README.md) |
| 11.9 | 測試與除錯 | 測試策略、除錯工具與驗證方法 | [實驗室 08](./11-MCPServerHandsOnLabs/08-Testing/README.md) |
| 11.10 | VS Code 整合 | 設定 VS Code MCP 整合與 AI 聊天使用 | [實驗室 09](./11-MCPServerHandsOnLabs/09-VS-Code/README.md) |
...
學習 MCP 最令人興奮的部分之一是看到你的程式技能逐步成長。我們設計的程式範例從簡單開始，隨著你理解的加深而變得更複雜。以下是我們介紹概念的方式——使用易於理解但展現真實 MCP 原則的程式碼，你不僅會了解程式碼做了什麼，還會明白為什麼這樣結構化，以及它如何融入更大的 MCP 應用中。
...
本儲存庫包含多項資源，幫助你有效導航與學習：

### 學習指南

提供一份完整的 [學習指南](./study_guide.md)，幫助你有效瀏覽本儲存庫。此視覺化課程地圖展示所有主題的連結，並提供如何有效使用範例專案的指引。對於喜歡從整體視角學習的視覺型學習者特別有幫助。
...
我們維護一份詳細的 [更新日誌](./changelog.md)，追蹤課程材料的所有重要更新，讓你隨時掌握最新改進與新增內容。
- 新增內容
- 結構調整
- 功能改進
- 文件更新
...
### [MCP Dev Days 2025 年 7 月](https://developer.microsoft.com/en-us/reactor/series/S-1563/)
#### [➡️隨選觀看 - MCP Dev Days](https://developer.microsoft.com/en-us/reactor/series/S-1563/)
準備好參加為期兩天的深度技術洞察、社群連結與實作學習的 MCP Dev Days 虛擬活動，專注於 Model Context Protocol (MCP)——這個連結 AI 模型與其依賴工具的新興標準。
您可以透過我們的活動頁面註冊觀看 MCP Dev Days：https://aka.ms/mcpdevdays。

#### [第 1 天：MCP 生產力、開發工具與社群：](https://developer.microsoft.com/en-us/reactor/series/S-1563/)

專注於賦能開發者在開發工作流程中使用 MCP，並慶祝精彩的 MCP 社群。我們將與 Arcade、Block、Okta 和 Neon 等社群成員及合作夥伴一同探討他們如何與微軟合作，打造開放且可擴充的 MCP 生態系。實際示範涵蓋 VS Code、Visual Studio、GitHub Copilot 及熱門社群工具
實用且具情境導向的開發工作流程
社群主導的議程與見解
無論您是剛開始接觸 MCP，或已經在使用 MCP 建置，第一天都將帶來啟發與可行的收穫。

#### [第 2 天：自信建置 MCP 伺服器](https://developer.microsoft.com/en-us/reactor/series/S-1563/)

專為 MCP 建置者設計。我們將深入探討建立 MCP 伺服器的實作策略與最佳實務，並將 MCP 整合到您的 AI 工作流程中。
...
如果您是開發者、工具建置者或 AI 產品策略師，這一天將充滿您所需的見解，助您打造可擴展、安全且面向未來的 MCP 解決方案。

### MCP 2025 年 8 月訓練營
透過密集的影片課程學習如何建立 MCP 伺服器、整合 VS Code，並根據 MCP 初學者課程內容專業部署於 Azure。帶走實用技能，掌握大型企業已在使用的技術。
...
### 一起用 C# 學 MCP - 教學系列
讓我們一起了解 Model Context Protocol (MCP)，這是一個先進框架，旨在標準化 AI 模型與客戶端應用程式之間的互動。透過這個適合初學者的課程，我們將介紹 MCP 並引導您建立第一個 MCP 伺服器。
...
恭喜！您剛踏出令人興奮的第一步，這將擴展您的程式設計能力，並連結到 AI 開發的最前沿。

### 您已經完成的部分

透過閱讀本介紹，您已開始建立 MCP 知識基礎。您了解 MCP 是什麼、為何重要，以及本課程如何支持您的學習旅程。這是重要的成就，也是您在此關鍵技術領域專業知識的起點。

### 未來的冒險

隨著您進入各模組，請記得每位專家都曾是初學者。現在看似複雜的概念，隨著練習與應用將變得駕輕就熟。每一步小進展都將累積成強大的能力，伴隨您整個開發生涯。

### 您的支援網絡

您將加入一個熱衷於 MCP 且樂於助人的學習者與專家社群。無論您遇到程式挑戰或想分享突破，社群都會支持您的旅程。

如果您卡關或有任何關於 AI 應用開發的問題，歡迎加入與其他學習者及經驗豐富開發者的 MCP 討論。這是一個歡迎提問且自由分享知識的支持性社群。
...
您的 MCP 冒險現在開始！從模組 0 開始，深入您的第一個 MCP 實作體驗，或探索範例專案，看看您將會建置什麼。請記住，每位專家都是從您現在的位置開始，憑藉耐心與練習，您將驚訝於自己的成就。

歡迎來到 Model Context Protocol 開發的世界。讓我們一起打造驚人的作品！

## 🤝 為學習社群貢獻

本課程因像您這樣的學習者貢獻而日益茁壯！無論是修正錯字、建議更清楚的說明，或新增範例，您的貢獻都幫助其他初學者成功。

感謝 Microsoft Valued Professional [Shivam Goyal](https://www.linkedin.com/in/shivam2003/) 貢獻程式碼範例。

貢獻流程設計為友善且支持性強。大多數貢獻需簽署貢獻者授權協議 (CLA)，但自動化工具會順利引導您完成流程。

## 📜 開源學習

整個課程皆採用 MIT [LICENSE](../../LICENSE) 授權，您可以自由使用、修改及分享。這支持我們讓 MCP 知識普及給全球開發者的使命。
...
*準備好開始您的 MCP 旅程了嗎？從 [模組 00 - MCP 介紹](./00-Introduction/README.md) 開始，踏出 Model Context Protocol 開發世界的第一步！*
